...
# ggplot(release_gdp, aes(x = year, y = gdp)) + 
#   geom_line()
```
...
```{r}
poly_mod <- lm(release_sum_n ~ poly(lag_release, 3) + poly(year, 3) + poly(oil_kg_n, 3) + poly(gdp, 3), data = release_lag)
summary(poly_mod)
```
...
```{r}
locfit1 <- locfit(release_sum_n ~ lag_release + year + oil_kg_n + gdp, data = train)
summary(locfit1)
```
...
```{r}
alpha_vals <- seq(0.1, 1, length.out=50) ##values to try
gcv_2 <- gcvplot(release_sum_n ~ lag_release + year + oil_kg_n + gdp, data=release_lag, alpha=alpha_vals) %>%  
  summary() %>%   
  as_tibble() %>%
...
```{r}
ggplot(gcv_2, aes(x=alpha, y=GCV)) +
  geom_point()
```
...
```{r}
locfit2 <- locfit(release_sum_n ~ lag_release + year + oil_kg_n + gdp, data = train, alpha = 2)

fit_pred2 <- predict(locfit2, newdata = test, alpha = 2)  ##prediction of already trained locfit model using new
...
```{r}
poly_mod2 <- lm(release_sum_n ~ poly(lag_release, 3) + poly(year, 3) + poly(oil_kg_n, 3) + poly(gdp, 3), data = train)
fit_pred0 <- predict(poly_mod2, newdata = test) 
mse_poly <- mean((test_df$release - fit_pred0)^2)         #difference between observed data in test and predicted data squared and then averaged.
mse_poly
```
